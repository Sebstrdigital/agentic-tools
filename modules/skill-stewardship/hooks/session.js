...
function shouldSuggest(session) {
  if (session.skillUsed || session.slashSkillUsed || session.explicitSkillUsed || session.suggestionWritten) return false;
  return (
...
function main() {
  ...
  if (name === "UserPromptSubmit") {
    session.turns += 1;
    if (looksLikeExplicitSkillInvocation(input)) {
      session.slashSkillUsed = true;
      session.explicitSkillUsed = true;
    }
  }

  if (name === "UserPromptExpansion" && looksLikeExplicitSkillInvocation(input)) {
    session.slashSkillUsed = true;
    session.explicitSkillUsed = true;
  }
